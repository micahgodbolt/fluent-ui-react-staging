...
function createShorthandFromValue<P>({
  ...
  const mappedHTMLProps: ShorthandProp = mappedProps[overrideProps.as || defaultProps.as];

  // Map prop for primitive value
  if (valIsPrimitive || valIsReactElement) {
    // TODO: (ken) there has to be a better way to handle this - https://github.com/microsoft/TypeScript/issues/31661
